    ...
    <article class="art-card">
      <button class="art-image-button" type="button" data-painting="${i}" aria-label="View ${escapeHTML(p.title)}">
        <div class="art-image-wrap">
          <img src="${p.image}" alt="${escapeHTML(p.title)}" width="${parseInt(p.size.split('×')[0])}" height="${parseInt(p.size.split('×')[1])}" loading="${i === 0 ? 'eager' : 'lazy'}" fetchpriority="${i === 0 ? 'high' : 'auto'}">
